...
export class EmployeeReportHandledComponent implements OnInit {
  ...
  onSubmit(form: NgForm){
    this.employeeService.addEmployeeReport(this.newReport).pipe(tap((res)=>{this.getEmployeeReports(this.selectedAnimalOption)}),takeUntilDestroyed(this.destroyRef)).subscribe();
    this.addFormIsDisplay = !this.addFormIsDisplay;
    form.reset();
    };
