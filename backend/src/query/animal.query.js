const QUERYANIMALS = {
    ...
    SELECT_ANIMALS_HABITAT: 'SELECT animals.id, animals.firstname, images.image_url AS image_url, breeds.name AS breed, reports.health AS health FROM animals LEFT JOIN (SELECT * FROM images WHERE images.id = LAST_INSERT_ID()) as images ON images.id_animal = animals.id INNER JOIN habitats ON habitats.id = animals.id_habitat INNER JOIN breeds ON breeds.id = animals.id_breed LEFT JOIN (SELECT * FROM reports where health is not null and reports.id = last_insert_id()) as reports ON reports.id_animal = animals.id WHERE habitats.id = ?'
}
