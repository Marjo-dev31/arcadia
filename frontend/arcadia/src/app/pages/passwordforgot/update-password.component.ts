...
@Component({
    ...
    <form #form="ngForm" (ngSubmit)="onSubmit(form)">
        ...
        <input type="email" id="emailForNewPassword" name="emailForNewPassword" #emailForNewPassword="ngModel" [(ngModel)]="user.email" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" required>
        @if (emailForNewPassword.invalid && emailForNewPassword.touched) {
            <p class="alert">Un email est obligatoire</p>
        }
        <label for="newPassword"  >Nouveau mot de passe :</label>
        <input type="password" id="newPassword" name="newPassword" #newPassword="ngModel" [(ngModel)]="user.password" pattern="^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$" required>
