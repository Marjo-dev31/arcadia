...
export class HabitatsService {
    ...
    getHandleHabitats(): Observable<Habitat[]> {
        return this.http.get<Response<Habitat>>(`${this.url}/backoffice`).pipe(map((r)=>{
            if(r.data){
                return r.data
            } else {
                return []
    ...
    updateHabitat(habitat: Habitat): Observable<Response<Habitat>> {
        return this.http.put<Response<Habitat>>(this.url + '/' + habitat.id, habitat)
    };
    ...
    addComment(habitat: Habitat, id: string): Observable<Response<Habitat>> {
        return this.http.put<Response<Habitat>>(this.url + '/' + id + '/comment', habitat)
    }

    deleteComment(id:string): Observable<Response<Habitat>>{
        return this.http.delete<Response<Habitat>>(this.url + '/' + id + '/comment');
    }
