const QUERYANIMALS = {
    ...
    SELECT_ANIMALS_HABITAT: 'SELECT animals.id, animals.firstname, animals.id_habitat, animals.id_breed, images.image_url AS image_url, habitats.title AS habitat, breeds.name AS breed, veterinary_reports.health AS health FROM animals LEFT JOIN (SELECT * FROM images LIMIT 1) as images ON images.id_animal = animals.id LEFT JOIN habitats ON habitats.id = animals.id_habitat LEFT JOIN breeds ON breeds.id = animals.id_breed LEFT JOIN veterinary_reports ON veterinary_reports.id_animal = animals.id WHERE habitats.id = ?'
}
