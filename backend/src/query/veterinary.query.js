const QUERYVETERINARIES = {
    SELECT_REPORTS:
        'SELECT reports.id, food, grammage, DATE_FORMAT(date, "%d/%m/%Y") AS date, health, details_condition, id_user, id_animal FROM reports INNER JOIN users ON id_user = users.id WHERE id_animal = ? AND id_role = (select id FROM roles WHERE name = "Vétérinaire")',
